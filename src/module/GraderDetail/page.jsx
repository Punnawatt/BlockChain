...
export default function GraderDetail() {
  ...
  //     // Step 2: Send ETH to the Submission Wallet Address
  //     const submissionWalletAddress = submissions?.problem?.walletAddress;
  //     const stakeAmount = 0.9*(submissions?.problem?.stake);
  ...
  //         value: web3.utils.toWei(stakeAmount.toString(), "ether"), // Send ETH
  //         gas: 21000, // Set an appropriate gas limit for ETH transfer
  //       });
  //       console.log(`${stakeAmount} ETH sent to Submission Wallet Address:`, submissionWalletAddress);
  ...
  //       // Step 4: Delete the problem with the same problemName
  //       const response_3000 = await fetch("http://localhost:3000/problems");
  //       if (!response_3000.ok) throw new Error("Failed to fetch problems");
  //       const problems = await response_3000.json();

  //       const problemToDelete = problems.find(
  //         (problem) => problem.problemName === submissions.problem.problemName
  //       );

  //       if (problemToDelete) {
  //         await fetch(`http://localhost:3000/problems/${problemToDelete.id}`, {
  //           method: "DELETE",
  //         });
  //         console.log("Problem deleted successfully.");
  //       } else {
  //         console.log("No matching problem found to delete.");
  //       }
  ...
  try {
    ...
    if (submissionWalletAddress) {
      ...
      // Step 3: Fetch all submissions and delete those with the same problemName
      const response = await fetch("http://localhost:8000/submissions");
      if (!response.ok) throw new Error("Failed to fetch submissions");
      const submissionsList = await response.json();

      // Filter submissions that have the same problemName
      const submissionsToDelete = submissionsList.filter(
        (submission) => submission.problem.problemName === submissions.problem.problemName
      );

      // Delete each matching submission
      for (const submissionToDelete of submissionsToDelete) {
        const deleteResponse = await fetch(`http://localhost:8000/submissions/${submissionToDelete.id}`, {
          method: "DELETE",
        });

        if (deleteResponse.ok) {
          console.log(`Submission with ID: ${submissionToDelete.id} deleted.`);
        } else {
          console.error("Failed to delete submission:", deleteResponse.statusText);
        }
      }
      ...
      <div className="flex w-full h-full">
        <div className="flex w-1/2 flex-col h-full">
          ...
          <div className="border m-4 p-4 rounded border-black">
            ...
            <div className="text-xl font-bold mt-4">Submission Submited by :</div>
            <div className="text-lg">{submissions?.problem?.walletAddress}</div>
            {problemData && (
              <div className="text-xl font-bold mt-4">Problem Created by :</div>
            )}
        ...
        <div className="flex w-1/2 flex-col">
          ...
          <div className="flex flex-row">
           ...
           <button onClick={handleReject} className="border w-20 my-4 ml-4 bg-red-400 rounded h-10">
             Reject
           </button>
         </div>
        </div>
