...
//     const problem = { problemName, detail, stake, walletAddress };
//     setProblemName("OLLLLLL");
//       setDetail("BLABLA");
//       setStake("0000000");
//       setTestcases([]);
//     try {

//       const response = await axios.post("http://localhost:8000/submitions/", problem, {
//         headers: { "Content-Type": "application/json" },
//       });
//       console.log("Submit created successfully:", response.data);
//       alert("New Submition added successfully");
...
export default function SubmitCode() {
  ...
  const submitCode = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/submissions/", {problem, code}, {
        headers: { "Content-Type": "application/json" },
      });
      console.log("Submit created successfully:", response.data);
    ...
    <>
      ...
      <div className="flex w-full h-full">
        <div className="flex w-1/2 flex-col h-full">
          ...
          <div className="flex flex-col h-full">
            <div ref={editorRef} className="m-4 w-full h-96"></div> {/* Added w-full and h-96 for size */}
            <button onClick={runCode} className="border w-20 m-4 bg-blue-300 rounded h-10">Run Code</button>
            <button onClick={submitCode} className="border w-20 m-4 bg-blue-300 rounded h-10">Submit Code</button>
          </div>
          <div>
            {randomTestCases.length > 0 && (
              <>
                ...
                {randomTestCases.map((testCase, index) => (
                  <div key={index}>
                    ...
                    <div className="text-xl font-bold m-2">Output {index + 1}:</div>
                    <pre className="border rounded border-black h-full m-4 p-2">{testCase.output}</pre>
                  </div>
                ))}
              </>
            )}
        ...
        <div className="flex w-1/2 flex-col">
          ...
          <div className="pb-12">
            <div className="text-xl font-bold m-2">Output:</div>
            <pre className="border rounded border-black h-full m-4 w-5/6 p-2">{output}</pre>
          </div>
        </div>
      </div>
    </>
